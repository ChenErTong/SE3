...
public class LogAction extends ActionSupport implements ServletRequestAware {
	...
	public String login() {
		...
		if(!result.equals("failure")){
			...
			ActionContext.getContext().getSession().put("notify", "T");
	...
	@Action(value = "commits", results = { @Result(name = "success", type = "json", params = { "root", "history" }) })
	public String queryCommits() {
		history = logBL.queryCommits(ActionContext.getContext().getSession().get("login"));
		return SUCCESS;
	}
