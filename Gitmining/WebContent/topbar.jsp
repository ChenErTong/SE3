...
$(document).ready(function(){
	...
	if(myLogin != ""){
        ...
        $("#topbar").append('<li><a onclick="getCommits();"><i>' + myLogin + '</i></a></li>');
        $("#topbar").append('<li><a onclick="signout();"><p>Sign Out</p></a></li>');
		}else{
     ...
     function getCommits(){
    	var isNew = "<%=isNew%>";
		if(isNew == "T"){
			if(window.location.pathname == "/Gitmining/welcome"){
				$.notify({
					icon : 'ti-github',
					message : "Fulfill the form on the bottom.",
				}, {
					type : 'info',
					delay : 2000,
					placement : {
				...
				$.notify({
					icon : 'ti-github',
					message : "Please click here to fulfill your basic information.",
					url: '/Gitmining/welcome',
					target: "_self",
				}, {
					type : 'danger',
					delay : 0,
			...
			var action = "/Gitmining/log/commits";
	         $.ajax({
	             type: "GET",
	             url: action,
	             async : false,
	             ...
	             success: function(data){
	            	 var del = 2000;
	            	 var ty = "danger";
	            	 var msg = "You have no learning repositories.<br>Find what you are interested in and mark them.";
	            	 if(data != null && data.length > 0){
	            		 del = 0;
	            		 msg = "";
	            		 ty = "warning";
	            		 for(var i = 0; i < data.length; i++){
	            			 if(data[i].commits > 0){
	            				var method = "if(event.button == 2){openGithub('" + data[i].repo + "');}else if(event.button == 0){refreshHistory('" + data[i].repo + "');}return false;";
	            				msg += '<a style="color: #B98F3B;" onmousedown="'+method+ '">' + data[i].repo + " has " + data[i].commits + ' new commits.</a><br>';
	            			 }
	            		 }
	            		 if(msg == ""){
	            			 del = 2000;
	            			 ty = "info";
	            			 msg = "All your learning repositories are newest.";
	            		 }
	            	 }
	            	 $.notify({
						icon : 'ti-github',
						title : '<b style="margin-left: 33%;">Welcome Back</b>',
						message : msg,
					}, {
						type : ty,
						delay : del,
